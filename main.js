var gmail;
var lock_img_url =
  "https://cdn2.iconfinder.com/data/icons/circle-icons-1/64/unlocked-20.png";
...
var pmail_endpoint = "https://zksnarks.org";
...
var main = async function() {
  ...
  searchable_list = JSON.parse(
    localStorage["pmail.searchable_encrypted" + user.email]
  );
  storPrivkey = "pmail.privkey-" + user.email;
  storPubkey = "pmail.pubkey-" + user.email;
  if (
    localStorage.getItem(storPrivkey) != null &&
    localStorage.getItem(storPubkey) != null
  ) {
  ...
  var button_html =
    '<div class="T-I J-J5-Ji ash ptool" role="button" tabindex="0" aria-haspopup="true" aria-expanded="false" data-tooltip="Pmail" aria-label="Settings" style="user-select: none;"><img class="inboxsdk__button_iconImg" src="' +
    lock_img_url +
    '"></div>';
  ...
  gmail.observe.on("compose", function(compose, type) {
    if (user.pmail_active) {
      ...
      var button_html =
        '<div class="T-I J-J5-Ji ash ptool" role="button" tabindex="0" aria-haspopup="true" aria-expanded="false" data-tooltip="Pmail" aria-label="Pmail" style="user-select: none;"><img class="inboxsdk__button_iconImg" src="' +
        lock_img_url +
        '"></div>';
      gmail.tools.add_compose_button(
        ...
        function(temp) {
          if (window.ComposeEncrypted) {
            ...
            decrypt(ciphertext, user.privatekey, user.passphrase).then(function(
              plaintext
            ) {
              setTimeout(() => {
                window.ComposeEncrypted = false;
            ...
            var encryptedList = Promise.all(
              emails.map(elem => encryptEmail(plaintext, elem))
  ...
  gmail.observe.on("view_email", function(obj) {
    ...
    if (ciphertextList != "") {
      try {
        var cListObj = JSON.parse(
          ciphertextList.match(/^{[^}{]+(?=})/s)[0] + "}"
        );
        var ciphertext = decodeURIComponent(cListObj[user.email]);
        decrypt(ciphertext, user.privatekey, user.passphrase).then(function(
          plaintext
          ...
          var email_id =
            window.emailRef["$el"][0]["attributes"]["data-message-id"].value;
          if (!searchable_list.includes(email_id)) {
            createEncryptedIndex(plaintext, email_id);
          }
          setTimeout(() => {
...
function postPublicKey(email, publickey) {
  ...
  request.open("POST", pmail_endpoint + "/publickey", false);    // `false` makes the request synchronous
...
function getPublicKey(email) {
  var request = new XMLHttpRequest();
  request.open("GET", pmail_endpoint+"/publickey?email=" + email, false);   // `false` makes the request synchronous
  request.send(null);
...
async function createEncryptedIndex(plaintext_body, email_id) {
  ...
  var tok_email_id = encodeURIComponent(
    await encrypt(email_id, user.publickey)
  );
  ...
  request.open("POST", pmail_endpoint+"/search", true);             // `false` makes the request synchronous
  request.send(formData);
...
async function getIds(queryList) {
  ...
  request.open("PUT", pmail_endpoint+"/search", false);                 // `false` makes the request synchronous
  request.send(formData);
  ...
  if (request.status === 200) {
    ...
    if (obj != null) {
      for (var j = 0; j < obj.length; j++) {
        var deID = await decrypt(
          decodeURIComponent(obj[j]),
          user.privatekey,
          user.passphrase
        );
        decList.add(deID);
...
function getUserNumber() {
  ...
  for (var i in logged_in_users) {
    if (logged_in_users[i].email == username) return logged_in_users[i].index;
  }
}
/**
